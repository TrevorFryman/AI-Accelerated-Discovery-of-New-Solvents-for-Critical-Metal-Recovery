"""
ChemBERTa Embedding Pipeline - Baseline XGBoost Model

Loads descriptors/ChemBERTa_Features.csv and trains a baseline XGBoost
regression model to predict G-score from ChemBERTa embeddings.
...
5. Prediction (parity) plots and residual plots

This pipeline is standalone (ChemBERTa embeddings only) and uses an
...
the Morgan Fingerprint and RDKit Descriptor pipelines.

Outputs are saved to results/ChemBERTa/.
"""
...
RESULTS_DIR = BASE_DIR / "results" / "ChemBERTa"
...
def main():
    ...
    plt.ylabel("R2 score")
    plt.title("Learning Curve - ChemBERTa Embedding XGBoost Baseline")
    plt.legend(loc="best")
    plt.tight_layout()
    ...
    if final_train_r2 < 0.5 and final_val_r2 < 0.5:
        ...
        fit_explanation = (
            ...
            f"enough signal from the ChemBERTa embedding features."
        )
    elif gap > 0.3:
    ...
    # 5. Prediction (parity) plots and residual plots (test set + OOF)
    fig, axes = plt.subplots(2, 2, figsize=(12, 10))

    # Prediction plot - test set
    ...
    axes[0, 0].set_title(f"Prediction Plot - Test Set (R2={r2_test:.3f})")
    axes[0, 0].legend()
    ...
    axes[1, 0].set_title(f"Prediction Plot - 5-Fold OOF (R2={r2_oof:.3f})")
    axes[1, 0].legend()
    ...
    plt.tight_layout()
    plt.savefig(RESULTS_DIR / "prediction_residual_plots.png", dpi=150)
    plt.close()

    # Save summary report
    report_lines = []
    report_lines.append("# ChemBERTa Embedding Baseline Model - Results\n\n")
    ...
    report_lines.append(f"- Features: {len(feature_cols)} ChemBERTa embedding dimensions\n")
    ...
    report_lines.append("- `results/ChemBERTa/learning_curve.png`\n")
    report_lines.append("- `results/ChemBERTa/prediction_residual_plots.png`\n")

    (REPORTS_DIR / "ChemBERTa_Baseline_Results.md").write_text("".join(report_lines), encoding="utf-8")

    print(f"\nResults saved to {RESULTS_DIR}")
    print(f"Report saved to {REPORTS_DIR / 'ChemBERTa_Baseline_Results.md'}")
