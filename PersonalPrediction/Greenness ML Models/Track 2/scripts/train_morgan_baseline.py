"""
Morgan Fingerprint Pipeline - Baseline XGBoost Model

Loads descriptors/Morgan_Features.csv and trains a baseline XGBoost
regression model to predict G-score from Morgan fingerprint bits.
...
This pipeline is standalone (Morgan fingerprints only) and uses an
...
the RDKit-descriptor and ChemBERTa pipelines.

Outputs are saved to results/Morgan/.
"""
...
RESULTS_DIR = BASE_DIR / "results" / "Morgan"
...
def main():
    ...
    plt.ylabel("R2 score")
    plt.title("Learning Curve - Morgan Fingerprint XGBoost Baseline")
    plt.legend(loc="best")
    plt.tight_layout()
    ...
    if final_train_r2 < 0.5 and final_val_r2 < 0.5:
        ...
        fit_explanation = (
            ...
            f"enough signal from the Morgan fingerprint features."
        )
    elif gap > 0.3:
    ...
    # Save summary report
    report_lines = []
    report_lines.append("# Morgan Fingerprint Baseline Model - Results\n\n")
    ...
    report_lines.append(f"- Features: {len(feature_cols)} Morgan fingerprint bits (radius=2, nBits=2048)\n")
    ...
    report_lines.append("- `results/Morgan/learning_curve.png`\n")
    report_lines.append("- `results/Morgan/parity_residual_plots.png`\n")

    (REPORTS_DIR / "Morgan_Baseline_Results.md").write_text("".join(report_lines), encoding="utf-8")

    print(f"\nResults saved to {RESULTS_DIR}")
    print(f"Report saved to {REPORTS_DIR / 'Morgan_Baseline_Results.md'}")
