...
def train_and_evaluate_descriptor_based_rf():
    # 1. Setup paths and directories
    dataset_path = os.path.join('data', 'RDKitDescriptorGeneration', 'DES_RDKit_Features.csv')
    results_dir = os.path.join('results', 'DescriptorBasedRandomForest')
    ...
    # Define feature columns: all RDKit descriptors + specified additional features
    rdkit_descriptors = [
    ...
    additional_features = [
        ...
        'Type of DES', 'Number of components', 'Phase diagram (Yes/No)'
    ]
    
    feature_cols = rdkit_descriptors + additional_features
    ...
    print(f"  RDKit Descriptors: {rdkit_descriptors}")
    ...
    # 6. 5-Fold Cross-Validation with Best Hyperparameters
    print("Executing 5-Fold Cross-Validation with best hyperparameters...")
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X, y)):
        ...
        # Clone the best pipeline to ensure independence of folds
    ...
    metrics_summary = {
        'best_hyperparameters': {str(k): v for k, v in grid_search.best_params_.items()},
        'best_cv_r2_score': float(grid_search.best_score_),
    ...
    # 7. Train Final Model on Complete Dataset
    print("Training final Descriptor-Based Random Forest model on complete dataset...")
    ...
    plt.title('Feature Importances (Descriptor-Based Random Forest Regressor)', fontsize=14, pad=15, fontweight='bold')
    ...
    # 9. Create Actual vs Predicted Plot
    plt.figure(figsize=(8, 7))
    
    # Scatter plot of actual vs OOF predicted
    sns.scatterplot(x=y, y=oof_predictions, alpha=0.6, color='steelblue', edgecolor='w', s=60)
    ...
    plt.title('Actual vs. Predicted Melting Temperature (5-Fold CV OOF)\nDescriptor-Based Random Forest', 
    ...
    model_path = os.path.join('models', 'descriptor_based_random_forest.pkl')
    with open(model_path, 'wb') as f:
        pickle.dump(best_pipeline, f)
    print(f"Saved serialized model pipeline to: {model_path}")
    
    # 11. Save top feature importances to CSV
...
if __name__ == '__main__':
    train_and_evaluate_descriptor_based_rf()
